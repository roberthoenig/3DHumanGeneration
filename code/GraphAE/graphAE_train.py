...
def train_one_iteration(param, model, optimizer, pc_lst, epoch, iteration):
    optimizer.zero_grad()
    # start=datetime.now()
    in_pc_batch = Dataloader.get_random_pc_batch_from_pc_lst_torch(pc_lst, param.neighbor_id_lstlst,
    ...
    if iteration % 10 == 0:
        ...
        param.logger.add_scalars('Train_loss_with_weight', {'Loss_pose': (loss_pose * param.w_pose).item()},
                                 total_iteration)
...
def evaluate(param, model, pc_lst, epoch, template_plydata, suffix, log_eval=True):
    ...
    while (n < (pc_num - 1)):
        ...
        geo_error_sum = geo_error_sum + model.compute_geometric_mean_euclidean_dist_error(pcs_torch,
                                                                                          out_pcs_torch) * batch

        if (n == 0):
            out_pc = np.array(out_pcs_torch[0].data.tolist())
            gt_pc = np.array(pcs_torch[0].data.tolist())
            Dataloader.save_pc_into_ply(template_plydata, out_pc,
                                        param.write_tmp_folder + "epoch%04d" % epoch + "_out_" + suffix + ".ply")
            Dataloader.save_pc_into_ply(template_plydata, gt_pc,
                                        param.write_tmp_folder + "epoch%04d" % epoch + "_gt_" + suffix + ".ply")
        n = n + batch
